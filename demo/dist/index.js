(() => {
  ...
  var solid = (obj, name, value, enumerable2 = true, configurable = false) => {
    return Object.defineProperty(obj, name, { enumerable: enumerable2, value, configurable });
  };
  var solids = (obj, namedValues, enumerable2 = true, configurable = false) => {
    for (const name in namedValues) {
      const value = namedValues[name];
      solid(obj, name, value, enumerable2, configurable);
    }
    return obj;
  };
  ...
  var normalizeFileName = (fileName, extension, fallback = "file") => {
  ...
  var createTextBlob = (content, mimeType = "text/plain", charset = "utf-8") => {
  ...
  var downloadObjectUrl = (url, fileName) => {
  ...
  var buildAccept = (extension, mimeType) => {
    const accept = [];
    const normalizedExt = normalizeExtension(extension);
    const normalizedMimeType = String(mimeType ?? "").trim();
    if (normalizedExt) {
      accept.push(`.${normalizedExt}`);
    }
    if (normalizedMimeType) {
      accept.push(normalizedMimeType);
    }
    return accept.join(",");
  };
  var createHiddenInput = (extension, mimeType, multiple = false) => {
  ...
  var pickFiles = (input, cancelCheckDelay = 300) => {
  ...
  var readFileAsText = (file, charset = "utf-8") => {
    return new Promise((resolve, reject) => {
  ...
  var loadTexts = async (input, charset = "utf-8", cancelCheckDelay = 300) => {
    try {
  ...
  var loadFiles = async (_io, multiple = false) => {
  ...
  var saveFile = (_io, data, fileName) => {
  ...
  var FileIO = class {
     ...
     */
    save(data, fileName) {
      return saveFile(this, data, fileName);
    }
    /**
     ...
     */
    async loadMany() {
      return loadFiles(this, true);
    }
  };
  var index_default = FileIO;

  // demo/src/index.js
  var fsInt = new index_default();
  window.fsInt = fsInt;
})();
